<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Nouvelle politique SLA') }}</h2>
    </x-slot>

    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white p-6 shadow-sm rounded-lg">
                <form method="POST" action="{{ route('sla-policies.store') }}" class="space-y-4">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select id="work_order_type" name="work_order_type" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                                ...
                                <option value="maintenance" @selected(old('work_order_type') === 'maintenance')>Maintenance</option>
                                <option value="demande_client" @selected(old('work_order_type') === 'demande_client')>Demande client</option>
                                <option value="preventif" @selected(old('work_order_type') === 'preventif')>Préventif</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <x-text-input id="response_time_minutes" name="response_time_minutes" type="number" min="1" class="mt-1 block w-full" :value="old('response_time_minutes')" required />
                            <x-input-error :messages="$errors->get('response_time_minutes')" class="mt-2" />
                        </div>
                        <div>
                            <x-input-label for="resolution_time_minutes" value="Délai de résolution (minutes)" />
                            <x-text-input id="resolution_time_minutes" name="resolution_time_minutes" type="number" min="1" class="mt-1 block w-full" :value="old('resolution_time_minutes')" required />
                            <x-input-error :messages="$errors->get('resolution_time_minutes')" class="mt-2" />
                        </div>
                    </div>
