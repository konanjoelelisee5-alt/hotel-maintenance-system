            ...
            <div class="bg-white p-6 shadow-sm rounded-lg">
                <form method="POST" action="{{ route('suppliers.update', $supplier) }}" class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea id="address" name="address" rows="2" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">{{ old('address', $supplier->address) }}</textarea>
                    </div>

                    <div>
                        <x-input-label for="notes" value="Notes" />
                        <textarea id="notes" name="notes" rows="2" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">{{ old('notes', $supplier->notes) }}</textarea>
                    </div>

                    <div class="flex justify-end gap-3">
                        <a href="{{ route('suppliers.show', $supplier) }}" class="px-4 py-2 text-sm text-gray-600 hover:underline">Annuler</a>
